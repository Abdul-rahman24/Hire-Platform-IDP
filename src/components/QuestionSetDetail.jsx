...
function LocalSkeletonRow() {
  return (
    <tr className="animate-pulse">
      <td className="px-5 py-4"><div className="h-3.5 bg-slate-100 rounded w-16" /></td>
      <td className="px-5 py-4"><div className="h-3.5 bg-slate-100 rounded w-64" /></td>
      <td className="px-5 py-4"><div className="h-3.5 bg-slate-100 rounded w-8" /></td>
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-6">
        ...
        <div className="flex items-center space-x-2 self-stretch md:self-auto justify-end flex-wrap gap-y-2">
          ...
          <button
            ...
            className="flex items-center px-3.5 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg font-semibold text-xs hover:bg-slate-50 transition-colors shadow-sm justify-center"
          >
            <svg className="w-3.5 h-3.5 mr-1.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
            Import CSV
          </button>
          <button
            onClick={onAddQuestion}
            className="flex items-center px-4 py-2 bg-[#0B4A99] text-white rounded-lg font-semibold text-xs hover:bg-[#083A78] transition-colors shadow-sm justify-center"
          >
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                >
                  ...
                  {isCodingSet && (
                    <td className="px-5 py-4 text-xs font-semibold text-slate-700 capitalize">
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex items-center justify-end space-x-1">
                      ...
                      <button
                        ...
                        className="text-slate-400 hover:text-[#0B4A99] hover:bg-blue-50 transition-colors p-2 rounded-xl flex items-center justify-center"
                      ...
                      <button
                        ...
                        title="Edit Question"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                      </button>
                      {/* Delete Button */}
                      <button
                        onClick={() => onDeleteQuestion(q)}
                        className="text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-colors p-2 rounded-xl flex items-center justify-center"
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden z-10">
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto text-xs">
              <div>
                ...
                <p className="text-slate-800 font-medium text-sm leading-relaxed whitespace-pre-wrap">{viewQuestionModal.question || viewQuestionModal.text}</p>
              </div>

              {isCodingSet ? (
                /* Coding Set View Details */
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Coding Metadata</p>
                  <div className="bg-slate-50 border border-slate-200/60 p-3.5 rounded-xl space-y-2 font-semibold">
                    <div className="flex justify-between items-center text-slate-600">
                      <span>Programming Language</span>
                      <span className="bg-blue-50 text-[#0B4A99] border border-blue-100 px-2.5 py-0.5 rounded-md text-[10px] capitalize">
                        {viewQuestionModal.language || 'python'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {['A', 'B', 'C', 'D'].map((lbl, idx) => {
                      ...
                      const optText = getOptTextVal(rawOpt);
                      ...
                      return (
                        <div key={lbl} className={`p-2.5 rounded-xl border flex items-center justify-between ${isCorrect ? 'bg-green-50 border-green-200 text-green-800 font-semibold' : 'bg-slate-50 border-slate-100 text-slate-700'}`}>
                          ...
                          {isCorrect && <span className="text-[10px] font-bold text-green-600 hidden sm:inline-block">✓ Correct Answer</span>}
                        </div>
                      );
                    })}
                  </div>
      ...
      <ImportCSVModal
        ...
        onImport={(importedQuestionsList) => {
          setIsImportOpen(false);
          if (onImportQuestions) {
            onImportQuestions(importedQuestionsList);
          }
        }}
        setType={set?.setType || 'MCQ'}
      />
    </div>
  );
}
