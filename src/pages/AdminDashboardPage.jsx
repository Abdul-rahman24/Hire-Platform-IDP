  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        className="flex items-center justify-between w-48 px-3.5 py-2 bg-white border border-slate-200 hover:border-slate-300 rounded-xl text-xs font-bold text-slate-600 shadow-[0_1px_2px_0_rgba(0,0,0,0.03)] hover:bg-slate-50/50 transition-all cursor-pointer"
      >
        <span>{options.find(o => o.value === value)?.label}</span>
        <svg className={`w-3.5 h-3.5 ml-2 text-slate-400 transition-transform duration-200 ${isOpen ? 'rotate-180 text-[#0B4A99]' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      {isOpen && (
        <div className="absolute right-0 top-full mt-1.5 w-56 bg-white border border-slate-200 rounded-xl shadow-xl p-1.5 z-50">
          {options.map(opt => (
            <button 
              key={opt.value} 
  ...
  return (
    <div className="w-full space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {loading ? (
          Array(4).fill(0).map((_, i) => <StatMini key={i} loading={true} />)
        ) : (
          <>
            <StatMini
      ...
      {/* ── Row 2: Test Performance Ranking Table ── */}
      <motion.div 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <motion.div 
          ...
          className="lg:col-span-7 bg-white border border-slate-200/80 rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] p-4.5 flex flex-col"
        >
          <div className="mb-6">
        ...
        <motion.div 
          ...
          className="lg:col-span-5 bg-white border border-slate-200/50 rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.04)] p-4.5"
        >
          <div className="mb-6">
      ...
      {/* ── Row 4: Test Performance Chart ── */}
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, delay: 0.7 }}
        className="w-full bg-white border border-slate-200/80 rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] p-6"
      >
